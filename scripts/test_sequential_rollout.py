#!/usr/bin/env python3
"""Test sequential_rollout strategy on a single network.

This script tests only the sequential_rollout strategy on DREAM4_10_1
to monitor its progress and performance.
"""
...
def test_sequential_rollout():
    """Test sequential_rollout strategy on DREAM4_10_1."""
    print("=" * 80)
    print("SEQUENTIAL ROLLOUT TEST - DREAM4_10_1")
    print("=" * 80)
    ...
    # Print sample scores
    sample_scores = list(edge_scores.values())[:10]
    print(f"Sample scores: {sample_scores}")
    print(f"Score range: [{min(edge_scores.values()):.6f}, {max(edge_scores.values()):.6f}]")
    print(f"Mean score: {np.mean(list(edge_scores.values())):.6f}")
    print(f"Std score: {np.std(list(edge_scores.values())):.6f}")
    ...
    strategies = ["self_attention", "tf_to_target", "combined_best"]
    comparison_results = {}
    ...
    for strategy in strategies:
        ...
        comparison_results[strategy] = {
            "aupr": metrics_cmp["aupr"],
            "auroc": metrics_cmp["auroc"],
            "time": elapsed,
        }
    ...
    all_strategies = [("sequential_rollout", metrics, training_time)] + [
        (s, comparison_results[s], comparison_results[s]["time"]) for s in strategies
    ]

    for strat, met, t in sorted(all_strategies, key=lambda x: x[1]["aupr"], reverse=True):
