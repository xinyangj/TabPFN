#!/usr/bin/env python
"""Retrain interpretation model on v3 cache (1591 dims, all 6 categories).

Trains on v3 cache only (high-feature, 10-150 features, 1591 dims).
Supports training on subset of categories or all 6.

Usage:
    python scripts/retrain_v3.py --device cuda:0
    python scripts/retrain_v3.py --device cuda:0 --config deep
    python scripts/retrain_v3.py --device cuda:0 --categories all  # all 6 cats (1591d)
    python scripts/retrain_v3.py --device cuda:0 --categories v2   # 4 cats (691d)
"""
...
def main():
    ...
    parser.add_argument("--config", default="deep", choices=["narrow", "deep"])
    ...
    input_categories, expected_dim = CATEGORY_SETS[args.categories]
    logger.info(f"Using device: {device}, config: {args.config}, categories: {args.categories}")
    logger.info(f"Architecture: {config['hidden_dims']}, LR: {config['lr']}")
    logger.info(f"Categories ({len(input_categories)}): {input_categories} -> {expected_dim}d")

    v3_dir = Path(args.v3_cache)

    out_path = Path(args.output) if args.output else OUT_DIR / f"best_v3_{args.config}_{args.categories}_model.pt"
    out_path.parent.mkdir(parents=True, exist_ok=True)

    # Load v3 cache
    logger.info("Loading v3 cache...")
    all_data = load_cache(v3_dir)
    logger.info(f"Total: {len(all_data)} datasets")
    ...
    checkpoint = {
        ...
        "data_sources": {
            "v3_cache": str(v3_dir),
            "n_v3": len(all_data) if 'all_data' in dir() else "unknown",
            "category_set": args.categories,
        },
    }
