#!/usr/bin/env python
"""MLP Hyperparameter Tuning — Phase 2.

Extends Phase 1 with:
- More learning rates (5e-5, 3e-4, 1e-3)
- Normalization (LayerNorm + ReLU/GELU)
- More MLP architectures (wider, deeper)

Groups A-C run independently; Group D combines best findings.

Usage:
    python scripts/tune_mlp_phase2.py [--group A|B|C|D|all]
"""
...
LOG_FILE = Path("results/interpretation_experiments/tuning_phase2_run.log")
_log_fh = None

def log(msg):
    """Log to both stdout and file with immediate flush."""
    global _log_fh
    import datetime
...
logger = type("L", (), {"info": staticmethod(log), "warning": staticmethod(log), "error": staticmethod(log)})()
...
GROUP_A_CONFIGS = [
    # LR sweep with best Phase 1 arch, no norm
    {"name": "A1_lr5e-5", "hidden_dims": [1024, 512, 256, 128], "lr": 5e-5, "norm": None, "activation": "relu"},
    {"name": "A2_lr3e-4", "hidden_dims": [1024, 512, 256, 128], "lr": 3e-4, "norm": None, "activation": "relu"},
    {"name": "A3_lr1e-3", "hidden_dims": [1024, 512, 256, 128], "lr": 1e-3, "norm": None, "activation": "relu"},
]

GROUP_B_CONFIGS = [
    # Normalization test with best Phase 1 settings
    {"name": "B1_LN_relu", "hidden_dims": [1024, 512, 256, 128], "lr": 1e-4, "norm": "layer", "activation": "relu"},
    {"name": "B2_LN_gelu", "hidden_dims": [1024, 512, 256, 128], "lr": 1e-4, "norm": "layer", "activation": "gelu"},
]

GROUP_C_CONFIGS = [
    # Architecture variations, lr=1e-4, no norm
    {"name": "C1_wide_deep", "hidden_dims": [2048, 1024, 512, 256], "lr": 1e-4, "norm": None, "activation": "relu"},
    {"name": "C2_wide", "hidden_dims": [1024, 1024, 512, 256], "lr": 1e-4, "norm": None, "activation": "relu"},
    {"name": "C3_uniform_deep", "hidden_dims": [512, 512, 512, 512, 256], "lr": 1e-4, "norm": None, "activation": "relu"},
    {"name": "C4_wide_shallow", "hidden_dims": [2048, 1024, 512], "lr": 1e-4, "norm": None, "activation": "relu"},
]

# Group D is dynamically generated from best of A-C (see main)
...
def train_and_evaluate(
    ...
    *, hidden_dims, lr, norm=None, activation="relu",
):
    ...
    logger.info(f"    Norm: {norm}, Activation: {activation}")
...
def run_group(configs, train_ds, val_ds, test_ds, existing_results=None):
    """Run a list of configs and return results dict."""
    ...
    for i, cfg in enumerate(configs):
        ...
        logger.info(f"  hidden_dims={cfg['hidden_dims']}, lr={cfg['lr']}, norm={cfg.get('norm')}, act={cfg.get('activation', 'relu')}")
        logger.info(f"{'='*60}")
        ...
        try:
            ...
            logger.info(
                ...
                f"epochs={metrics['n_epochs_trained']}, time={metrics['train_time_s']:.0f}s"
            )
        ...
        output_path = RESULTS_DIR / "tuning_phase2_results.json"
...
def print_summary(results):
    logger.info("\n" + "=" * 80)
    logger.info("PHASE 2 TUNING RESULTS SUMMARY")
    logger.info("=" * 80)
    logger.info(f"{'Config':<25s} {'AUROC':>8s} {'AUPR':>8s} {'ValLoss':>10s} {'Epochs':>7s} {'Time':>7s} {'Params':>10s}")
    logger.info("-" * 80)
    ...
    for name, r in results.items():
        if "error" in r:
            ...
            continue
        m = r["metrics"]
        ranked.append((m.get("auroc", 0), name, m))
    ...
    for auroc, name, m in sorted(ranked, reverse=True):
        logger.info(
            ...
            f"{m['train_time_s']:>6.0f}s {m['n_params']:>10,}"
        )

    if ranked:
        best_auroc, best_name, _ = max(ranked)
...
def main():
    ...
    parser.add_argument("--group", default="all", choices=["A", "B", "C", "D", "all"])
    args = parser.parse_args()

    logger.info("=" * 60)
    logger.info("MLP Hyperparameter Tuning — Phase 2")
    logger.info("=" * 60)
    logger.info(f"Device: {DEVICE}")
    logger.info(f"Batch size: {BATCH_SIZE}, Max epochs: {MAX_EPOCHS}, Patience: {PATIENCE}")
    logger.info(f"Running group(s): {args.group}")
    ...
    logger.info(f"Pre-tensorized in {time.time() - t_tensor:.1f}s — shapes: train={tuple(train_ds.features.shape)}, val={tuple(val_ds.features.shape)}, test={tuple(test_ds.features.shape)}")

    # Load existing results for incremental runs
    results_path = RESULTS_DIR / "tuning_phase2_results.json"
    existing = {}
    ...
    if args.group in ("A", "all"):
        logger.info("\n" + "=" * 60)
        logger.info("GROUP A: Learning Rate Sweep")
        logger.info("=" * 60)
        existing = run_group(GROUP_A_CONFIGS, train_ds, val_ds, test_ds, existing)

    if args.group in ("B", "all"):
        logger.info("\n" + "=" * 60)
        logger.info("GROUP B: Normalization Test")
        logger.info("=" * 60)
        existing = run_group(GROUP_B_CONFIGS, train_ds, val_ds, test_ds, existing)

    if args.group in ("C", "all"):
        logger.info("\n" + "=" * 60)
        logger.info("GROUP C: Architecture Variations")
        logger.info("=" * 60)
        existing = run_group(GROUP_C_CONFIGS, train_ds, val_ds, test_ds, existing)

    if args.group in ("D", "all"):
        logger.info("\n" + "=" * 60)
        logger.info("GROUP D: Best Norm + Best Arch Combinations")
        logger.info("=" * 60)

        # Find best architecture from C (or use default)
        best_arch = [1024, 512, 256, 128]
        best_arch_auroc = 0
        for name, r in existing.items():
            if name.startswith("C") and "metrics" in r:
                if r["metrics"].get("auroc", 0) > best_arch_auroc:
                    best_arch_auroc = r["metrics"]["auroc"]
                    best_arch = r["config"]["hidden_dims"]
        # Also compare with Phase 1 deeper baseline
        if best_arch_auroc < 0.6688:
            best_arch = [1024, 512, 256, 128]

        logger.info(f"  Best architecture for Group D: {best_arch}")

        group_d_configs = [
            {"name": "D1_best_LN_gelu_lr5e-5", "hidden_dims": best_arch, "lr": 5e-5, "norm": "layer", "activation": "gelu"},
            {"name": "D2_best_LN_gelu_lr1e-4", "hidden_dims": best_arch, "lr": 1e-4, "norm": "layer", "activation": "gelu"},
            {"name": "D3_best_LN_gelu_lr3e-4", "hidden_dims": best_arch, "lr": 3e-4, "norm": "layer", "activation": "gelu"},
        ]
        existing = run_group(group_d_configs, train_ds, val_ds, test_ds, existing)

    total_time = time.time() - t_total

    # Final summary
    ranked = print_summary(existing)
    logger.info(f"\nPhase 2 total time: {total_time/3600:.1f} hours")
